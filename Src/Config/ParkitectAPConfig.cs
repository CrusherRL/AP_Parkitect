...
namespace ArchipelagoMod.Src.Config
{
    class ParkitectAPConfig
    {
        ...
        public static void CreateConfig ()
        {
            ...
            Helper.Debug("[Challenge::CreateConfig] -> ParkitectAPConfig Created new config");
        }
        ...
        public static ParkitectAPConfig Load ()
        {
            ...
            try
            {
                ...
                {
                    Helper.Debug("[Challenge::Load] -> Invalid address given.");
                    return null;
                }

                Helper.Debug($"[Challenge::Load] -> Using server={self.Address}:{self.Port} player={self.Playername}");
                return self;
            }
            catch (Exception ex)
            {
                Helper.Debug($"[Challenge::Load] -> Manual parse failed: {ex.Message}");
                Helper.Debug($"[Challenge::Load] -> JSON: {jsonData}");
                return null;
            }
        }
